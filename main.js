...
*    and IndiaLyrics.net for hindi songs in indic script                  *
*                                                                         *
*   Copyright                                                             *
...
GEETURL = "http://indolyrics.heroku.com/search/gi?";
...
/* This function shows the HTML or XML formatted data to the Amarok lyrics canvas
*/
function onIndoLyricsReceived( response, title, redirects )
{
    try
    {
        if( response.length == 0 )
        ...
        {
            if( capture = /sorry/i.exec(response) ) {             //no lyrics then try without artist
                var newurl = GEETURL + "title=" + encodeURIComponent(title);
                var url = QUrl.fromEncoded( new QByteArray( newurl ), 1);
Amarok.debug( "request URL: " + url.toString() );
                new Downloader( url, new Function("response", "onIndoLyricsReceived(response, title, -1)") );
            else if( capture = /<table/i.exec(response) ) {       //got lyrics
                NEWHTML = HTML.replace( "{body}", response );
                ...
                for( var i = 0; i < items.length; i++ ) {
                    suggestion += "<suggestion artist=\"" + items[i].artist + "\" title=\"" + items[i].title + "\" url=\"" + items[i].url + "\" />\n";
                }
...
function getLyrics( artist, title, url )
{
    try
    {
        ...
        var album = Amarok.Engine.currentTrack().album.replace(/[\r\t\n]/g, "");
...
Amarok.debug( "request URL: " + newurl.toString() );
                new Downloader( newurl, new Function("response", "onIndoLyricsReceived(response, title, -1)") );
            }
            else {
                var newurl = GEETURL + "title=" + encodeURIComponent(title) + "&film=" + encodeURIComponent(album) ;
                var url = QUrl.fromEncoded( new QByteArray( newurl ), 1);
Amarok.debug( "request URL: " + url.toString() );
                new Downloader( url, new Function("response", "onIndoLyricsReceived(response, title, -1)") );
            }
